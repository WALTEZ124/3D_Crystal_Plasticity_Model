...
## Generate INP File
abaqus_6.11-2 cae noGUI=Create_INP_file_Cyclic.py

## Import JobName and sources' path from previous computation

EP_JobName_I=$(sed -n 1p EP_Cyc_job_details_I.txt).inp
EP_JobName_II=$(sed -n 1p EP_Cyc_job_details_II.txt).inp
EP_JobName_III=$(sed -n 1p EP_Cyc_job_details_III.txt).inp
