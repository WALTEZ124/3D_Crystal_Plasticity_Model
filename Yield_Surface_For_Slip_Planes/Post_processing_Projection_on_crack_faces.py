...
for slip_suffix in slip_systems_list :
    ...
    odb.close()
    file2=open(os.path.join( ResultsDir_Slip,'EV1_Cum_moy_%s_%s' % ( Job.I.PL.MonName , slip_suffix) ),'w') 
    ...
    time , EV1_Cum_tot, EV1_Cum_moy, EV1_Cum_quad = Extract_Cumulated_Strain_Field_Output(odb,'Step-1', dimensions)
    odb.close()
    file2=open(os.path.join( ResultsDir_Slip,'EV1_Cum_moy_%s_%s' % ( Job.II.PL.MonName , slip_suffix) ),'w') 
    file2.write('time ,  EV1 Cum arithmetic mean, EV1 Cum quadratic mean')
    file2.write(' \n' )
    for t in range(len(time)):
        file2.write('%30.20E   ' % time[t])
    ...
    time , EV1_Cum_tot, EV1_Cum_moy, EV1_Cum_quad = Extract_Cumulated_Strain_Field_Output(odb,'Step-1', dimensions)
    odb.close()
    file2=open(os.path.join( ResultsDir_Slip,'EV1_Cum_moy_%s_%s' % ( Job.III.PL.MonName , slip_suffix) ),'w') 
    file2.write('time ,  EV1 Cum arithmetic mean, EV1 Cum quadratic mean')
    file2.write(' \n' )
    for t in range(len(time)):
        file2.write('%30.20E   ' % time[t])
    ...
    file2.close()
    os.system('echo slip system %s: Total and plastic fields successfully dumped :p' % (slip_suffix) )
    #-------------------------------------------------------------------------------------------------
