...
PL_Star_Job  = { 'n_tot_steps' : n_tot_steps, 'n_init_steps' : 2 , 
					...
					'ini_inc' : dimensions.time_inc , 'min_inc' : 1e-6, 'max_inc' : 4 }
...
prop_ange_I_II = arctan(KI_Center/KII_Center)
KI_max  = KI_Center  + stab_rad * sin( init_prop_angle) 
KII_max = KII_Center + stab_rad * cos( init_prop_angle) 
...
for sec_prop_angle in Explored_Direction :
	KI_sec   =  KI_Center  + KRadius*sin( sec_prop_angle*np.pi/180 ) 
	KII_sec  =  KII_Center + KRadius*cos( sec_prop_angle*np.pi/180 ) 
	KIII_sec =  0.
...
prop_ange_I_III = arctan(KI_Center/KIII_Center)

KI_max   = KI_Center  + stab_rad * sin( init_prop_angle) 
KIII_max = KIII_Center + stab_rad * cos( init_prop_angle) 
...
for sec_prop_angle in Explored_Direction :
	KI_sec   =  KI_Center  + KRadius*sin( sec_prop_angle*np.pi/180 ) 
	KII_sec  =  0. 
	KIII_sec =  KIII_Center + KRadius*cos( sec_prop_angle*np.pi/180 ) 
...
prop_ange_II_III = arctan(KII_Center/KIII_Center)

KII_max  = KII_Center  + stab_rad * sin( init_prop_angle) 
KIII_max = KIII_Center + stab_rad * cos( init_prop_angle) 
...
for sec_prop_angle in Explored_Direction :
	KI_sec   =  0.5
	KII_sec  =  KII_Center  + KRadius*sin( sec_prop_angle*np.pi/180 ) 
	KIII_sec =  KIII_Center + KRadius*cos( sec_prop_angle*np.pi/180 ) 
