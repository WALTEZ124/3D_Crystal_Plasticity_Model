...
# -*- coding: utf-8 -*-
def Extract_Strain_Field_Output(odb, step, dimensions):
	listN_F_unsorted = dimensions.listN_F_unsorted
	listN_F_sorted   = dimensions.listN_F_sorted
	...
	for f in range(n_increments+1) :
		...
		if frameValue in list_increments :
			time +=[frameValue]
			Gamma_Field = step.frames[f].fieldOutputs['SDV7'].getSubset(region=SECTION_RIGHT)
			Gamma=[]
			EV1_Cum_Field = step.frames[f].fieldOutputs['SDV14'].getSubset(region=SECTION_RIGHT)
			EV1_Cum=[]
			for i,label in enumerate(listN_F_sorted):
				ind = listN_F_unsorted.index(label)
				Gamma += [ Gamma_Field.values[ind].data]
				EV1_Cum += [ EV1_Cum_Field.values[ind].data]
			Gamma_moy+= [np.mean(Gamma)]
			Gamma_quad += [np.linalg.norm(Gamma)/np.sqrt(listN_F_len)]
	...
	Gamma_tot = np.asarray(Gamma_tot)
	Gamma_tot = np.transpose(Gamma_tot)
	return time, EV1_Cum_tot, EV1_Cum_moy, EV1_Cum_quad, Gamma_tot, Gamma_moy, Gamma_quad
